...
class BaseModel(nn.Module):
    def __init__(self, 
        ...
        self._retrieval_augmented = retrieval_augmented
        if self._retrieval_augmented:
            assert retrieval_configs is not None, "retrieval-augmented mode requires a dataset with retrieval configurations"
            self._labelwise_retrieval = retrieval_configs["label_wise"]
    ...
    def inputs_to_device(self, inputs):
        if self._retrieval_augmented:
            X, y, retrieved_values, retrieved_lens = inputs
            X = X.to(self.device)
            y = y.float().unsqueeze(-1).to(self.device)
            retrieved_values = retrieved_values.to(self.device)
            retrieved_lens = retrieved_lens.int().to(self.device)
            self.batch_size = y.size(0)
            return X, y, retrieved_values, retrieved_lens
        else:
            X, y = inputs
            X = X.to(self.device)
            y = y.float().unsqueeze(-1).to(self.device)
    ...
    def predict_generator(self, data_generator):
        ...
        with torch.no_grad():
            ...
            for batch_index, batch_data in enumerate(data_generator):
                if self._retrieval_augmented:
                    assert batch_data[0].ndim == 3, \
                        "retrieval augmented mode requires input_shape like [Bx(1+K)xF]"
                else:
                    if batch_data[0].ndim == 3:
                        if batch_index == 0:
                            logging.info("[[WARNING]] dataloader provided retrieved samples but the model doesn't enable retrieval-augmentated prediction.")
                        batch_data[0] = batch_data[0][:, 0]
                        batch_data[1] = batch_data[1][:, 0]
                        batch_data = batch_data[:2] # remove the retrieved_values and retrieved_lens
                return_dict = self.forward(batch_data)
                y_pred.extend(return_dict["y_pred"].data.cpu().numpy().reshape(-1))
            y_pred = np.array(y_pred, np.float64)
            return y_pred
