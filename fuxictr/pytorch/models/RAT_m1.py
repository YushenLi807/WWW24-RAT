...
class RAT_m1(BaseModel):
    def __init__(self, 
                 ...
                 scale_dim = 4,
                 **kwargs):
        super(RAT_m1, self).__init__(feature_map, 
                                      model_id=model_id, 
                                      gpu=gpu, 
        ...
        self.intra_transformer = Transformer(embedding_dim, depth, num_heads, dim_head, embedding_dim*scale_dim, dropout)
        self.cross_transformer = Transformer(embedding_dim, depth, num_heads, dim_head, embedding_dim*scale_dim, dropout)
    ...
    def forward(self, inputs):
        ...
        x = self.cross_transformer(x)  # Bx(K+1)xd
        x = x[:,0]  # b d
