...
class EmbeddingDictLayer(nn.Module):
    ...
    def dict2tensor(self, embedding_dict, feature_source=None, feature_type=None):
        if feature_source is not None:
            ...
            return torch.stack(list(embedding_dict.values()), dim=-2)
...
class MergedEmbeddingLayer(nn.Module):
    def __init__(self, 
                 feature_map,
                 embedding_dim):
        super(MergedEmbeddingLayer, self).__init__()
        num_embedding = 0
        for _, feature_spec in feature_map.feature_specs.items():
            num_embedding += feature_spec["vocab_size"]
        self.embedding_layer = nn.Embedding(num_embedding, embedding_dim)
