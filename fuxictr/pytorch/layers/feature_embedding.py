...
class FeatureEmbeddingDict(nn.Module):
    ...
    def reset_parameters(self):
        ...
        for k, v in self.embedding_layers.items():
            if self.use_pretrain and "pretrained_emb" in self._feature_map.feature_specs[k]: # skip pretrained
                continue
            if "share_embedding" in self._feature_map.feature_specs[k] and v.weight.requires_grad == False:
                continue
