...
class FeatureEncoder(object):
    ...
    def read_csv(self, data_path):
        assert isinstance(data_path, (list, str)), \
            "require a string filename or a list of string filenames"
        all_cols = self.feature_cols + [self.label_col]
        dtype_dict = dict((x["name"], eval(x["dtype"]) if isinstance(x["dtype"], str) else x["dtype"]) 
                          for x in all_cols)
        if isinstance(data_path, list):
            logging.info("Reading file: " + ', '.join(data_path))
            ddf = pd.concat([pd.read_csv(path, dtype=dtype_dict, memory_map=True)
                             for path in data_path], ignore_index=True, copy=False)
        else:  # str
